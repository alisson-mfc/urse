...
def calc(str, key, files):
    ...
    for article in str["article_data"]:
        file = (
            article["gold"]["filename"].replace(" ", "_")
            if "gold" in article and "filename" in article["gold"]
            else "-"
        )
        ...
        random_sequence_generation = {
            ...
            "label": check_rct(BIAS_MAP[article["ml"]["bias"][0]["judgement"]], is_rct)[
                "label"
            ],
            "score": check_rct(BIAS_MAP[article["ml"]["bias"][0]["judgement"]], is_rct)[
                "score"
            ],
        }

        bias["random_sequence_generation"][trial] = check_rct(
            BIAS_MAP[article["ml"]["bias"][0]["judgement"]], is_rct
        )["score"]

        allocation_concealment = {
            "domain": article["ml"]["bias"][1]["domain"],
            "judgement": article["ml"]["bias"][1]["judgement"],
            "label": check_rct(BIAS_MAP[article["ml"]["bias"][1]["judgement"]], is_rct)[
                "label"
            ],
            "score": check_rct(BIAS_MAP[article["ml"]["bias"][1]["judgement"]], is_rct)[
                "score"
            ],
        }

        bias["allocation_concealment"][trial] = check_rct(
            BIAS_MAP[article["ml"]["bias"][1]["judgement"]], is_rct
        )["score"]

        blinding_of_participants_and_personnel = {
            "domain": article["ml"]["bias"][2]["domain"],
            "judgement": article["ml"]["bias"][2]["judgement"],
            "label": check_rct(BIAS_MAP[article["ml"]["bias"][2]["judgement"]], is_rct)[
                "label"
            ],
            "score": check_rct(BIAS_MAP[article["ml"]["bias"][2]["judgement"]], is_rct)[
                "score"
            ],
        }

        bias["blinding_of_participants_and_personnel"][trial] = check_rct(
            BIAS_MAP[article["ml"]["bias"][2]["judgement"]], is_rct
        )["score"]

        blinding_of_outcome_assessment = {
            "domain": article["ml"]["bias"][3]["domain"],
            "judgement": article["ml"]["bias"][3]["judgement"],
            "label": check_rct(BIAS_MAP[article["ml"]["bias"][3]["judgement"]], is_rct)[
                "label"
            ],
            "score": check_rct(BIAS_MAP[article["ml"]["bias"][3]["judgement"]], is_rct)[
                "score"
            ],
        }

        bias["blinding_of_outcome_assessment"][trial] = check_rct(
            BIAS_MAP[article["ml"]["bias"][3]["judgement"]], is_rct
        )["score"]
        count = count + 1
        response.append(
            {
...
def calc_score(part, n_samples):
    ...
    risco_vies = 0  # downgrade_risco_vies(part, n_samples)
...
def extractTotal(text):
    ...
    if listanpart:
        ...
        print("numeros separados", numeros_separados)
...
def comparators_calc(uid):
    ...
    for r in result:
        ...
        for i in data["article_data"]:
            total = total + float(
                i["ml"]["sample_size"] if check_numeric(i["ml"]["sample_size"]) else 10
            )
        ...
        for response in result1:
            (
                trial,
                downgrade_n_participantes,
                risco_vies,
                risco_vies_json,
                sample_size,
            ) = calc_score(response, total)
    ...
    for r in final_result:
        ...
        try:
            ...
            if t != [("?", "?")] and check_numeric(t[0][0]) and check_numeric(t[0][1]):
                ...
                final_result[r]["risco_vies_total"] = downgrade_risco_vies(
                    final_result[r]["part"], final_result[r]["sample"]["result"]
                )
        except Exception as e:
            logging.critical(e, exc_info=True)
            t = [("?", "?")]
        ...
        final_result[r]["final_score"] = (
            final_result[r]["risco_vies_total"]
            + final_result[r]["downgrade_n_participantes"]
            + final_result[r]["i2_score"]
            + amstar_score
        )
        ...
        final_result[r]["i2_json"]["heterogeneity"]["result"] = final_result[r][
            "i2_score"
        ]

        _json = dict()
        _json["values"] = [
            {"label": final_result[r]["outcome"], "value": final_result[r]["outcome"]},
            {
        ...
        if (
            (total == 0 or "?" in total)
            and t[0][0] != "?"
            and check_numeric(t[0][0])
            and check_numeric(t[0][1])
        ):
            _json["result"]["number_of_participants"]["total"] = int(t[0][0]) + int(
                t[0][1]
            )
            _json["result"]["number_of_participants"]["result"] = int(t[0][0]) + int(
                t[0][1]
            )
            ...
            if size_zero > 0:
                for i in _json["result"]["number_of_participants"]["items"].keys():
                    if (
                        _json["result"]["number_of_participants"]["items"][i] == 0
                        or _json["result"]["number_of_participants"]["items"][i]
                        == "???"
                    ):
